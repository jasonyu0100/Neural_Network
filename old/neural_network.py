...
class Network:
  ...
  def back_propogation(self,label,input_features,final_output_layer):
    for layer_index,layer in reversed(list(enumerate(self.layers))):
      for neuron_index,neuron in enumerate(layer.neurons):
        ...
        # cost_bias_derivative = -1 * neuron.cost_derivative * neuron.activation_derivative
        # neuron.stored_bias_delta += cost_bias_derivative

  def train_network(self,training_set,valid_set,learning_rate,epochs,batch_size,display_epoch):
    back_propogation_info = {'learning_rate':learning_rate,'batch_size':batch_size}
    for epoch_count in range(epochs):
      training_cost,training_correct = self.data_set_passover(training_set,back_propogation_info=back_propogation_info)
  ...
  def data_set_passover(self,data_set,back_propogation_info=None):
    cost = 0
    correct = 0
    for counter,data in enumerate(data_set):
      ...
      cost += current_cost
      if back_propogation_info != None:
        self.back_propogation(label,input_features,final_output_layer)
        if counter % back_propogation_info['batch_size'] == 0:
          self.update_network(back_propogation_info['learning_rate'])
    if back_propogation_info!=None:self.update_network(back_propogation_info['learning_rate'])
    return cost,correct
  ...
  def get_network_data(self,total_cost,total_correct,data_set_length):
    ...
    average_cost = round(float(total_cost) / float(data_set_length),3)
    return ("Average Cost: {} Accuracy: {}".format(average_cost,accuracy))
...
class Layer:
  ...
  def init_neurons(self,previous_layer_size):
    if self.neuron_weights == False: self.neuron_weights = [self.random_initialization(previous_layer_size) for neuron_index in range(self.size)]
  ...
  def random_initialization(self,previous_layer_size):
    return [1-2*random.random() for i in range(previous_layer_size)]
...
def main():
  #Network Information
  nn = Network([Layer(2,activation_function=sigmoid,input_size=2),
                Layer(2,activation_function=sigmoid)],
  ...
  nn.train_network(training_set,valid_set,learning_rate=0.01,epochs=100000,batch_size=1,display_epoch=5000)
